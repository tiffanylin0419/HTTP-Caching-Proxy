...
void * handle(void * info) {
  ...
  if (bytes_received<0){
    pthread_mutex_lock(&mutex1);
    logFile << clientdata->id << ": WARNING Invalid Request" << std::endl;
    pthread_mutex_unlock(&mutex1);
    return NULL;
  }
  ...
  //add
  pthread_mutex_lock(&mutex1);		
  logFile << clientdata->id << ": " << "\"" <<request.line<< "\"" << " from "<< clientdata->ip_address << " @ " << now_date.toPrintString();		
  pthread_mutex_unlock(&mutex1);
  const char * host = request.host.c_str();
  const char * port = request.port.c_str();
  int server_fd = setup_client(host,port);

  //connect
  if (request.method=="CONNECT"){
    pthread_mutex_lock(&mutex1);
    logFile << clientdata->id << ": "		<< "Requesting \"" << request.line << "\" from " << host << std::endl;		
    pthread_mutex_unlock(&mutex1);
    httpConnect(client_fd, server_fd, clientdata->id);
  }
  //post
    ...
    logFile << clientdata->id << ": "	<< "Requesting \"" << request.line << "\" from " << host << std::endl;		
    pthread_mutex_unlock(&mutex1);
    httpPost(client_fd, server_fd,request, clientdata->id);
  }
  //get
    ...
    if (cache.count(request.line) == 0){
      pthread_mutex_lock(&mutex1);		
      logFile << clientdata->id << ": not in cache" << std::endl;		
      pthread_mutex_unlock(&mutex1);
      //DRY
      std::string server_response = request_directly(client_fd, server_fd,request,clientdata->id);
      if(server_response==""){
        pthread_mutex_lock(&mutex1);		
        logFile << clientdata->id << ": ERROR server_response!" << std::endl;;		
        pthread_mutex_unlock(&mutex1);
        return NULL;
      }
      else if(server_response=="chunk"){
        pthread_mutex_lock(&mutex1);		
        logFile << clientdata->id << ": NOTE chunk!" << std::endl;;		
        pthread_mutex_unlock(&mutex1);
        return NULL;
      }
      ...
      //add a lot
      pthread_mutex_lock(&mutex1);
      logFile << clientdata->id  << ": " << "Received \"" << response.line << "\" from " << request.host << std::endl;		
      pthread_mutex_unlock(&mutex1);		
      pthread_mutex_lock(&mutex1);		
      logFile << clientdata->id  << ": Responding \"" << response.line << "\""<< std::endl;		
      pthread_mutex_unlock(&mutex1);		
      //std::cout<<"\n\n"<<response.input<<"\n";		
      //for log		
    ...
    //地一行在cache裡
      ...
      pthread_mutex_lock(&mutex1);		
      logFile << clientdata->id  << ": in cache, but expired at "<< response.expire_time.toPrintString() << std::endl;		
      pthread_mutex_unlock(&mutex1);
      //no-cache
      if(response.needRevalidate && !response.needCheckTime){
        std::cout<<"\n\nrevalidate suceed\n\n";
        pthread_mutex_lock(&mutex1);		
        logFile << clientdata->id  << ": NOTE Cache-Control: no-cache" << std::endl;		
        pthread_mutex_unlock(&mutex1);		
        pthread_mutex_lock(&mutex1);		
        logFile << clientdata->id  << ": in cache, requires validation" << std::endl;		
        pthread_mutex_unlock(&mutex1);
        if(revalidate(server_fd, client_fd, request, response, clientdata->id )==-1){return NULL;}
      }
      //must-revalidate
      else if(response.needRevalidate && response.needCheckTime){
        pthread_mutex_lock(&mutex1);		        
        logFile << clientdata->id  << ": NOTE Cache-Control: must-revalidate" << std::endl;		
        pthread_mutex_unlock(&mutex1);
        ...
        if((response.max_age_time.isEmpty() && response.expire_time.isEmpty()) ||
          ...
          pthread_mutex_lock(&mutex1);
          logFile << clientdata->client_fd  << ": in cache, requires validation" << std::endl;		
          pthread_mutex_unlock(&mutex1);
          ...
          logFile << clientdata->id  << ": Responding \"" << response.line << "\"" << std::endl;		
          pthread_mutex_unlock(&mutex1);
          if(send_client_cache_directly(client_fd, request,clientdata->id)==-1){return NULL;}
        }
      }
      ...
      //max-age or expire date
        ...
        pthread_mutex_lock(&mutex1);	
        logFile << clientdata->id << ": NOTE Cache-Control: max-age" << std::endl;	
        pthread_mutex_unlock(&mutex1);
      ...
      //public
        ...
        pthread_mutex_lock(&mutex1);		
        logFile << clientdata->id << ": NOTE Cache-Control: public" << std::endl;		
        pthread_mutex_unlock(&mutex1);
  ...
  //none
    ...
    error400(client_fd,clientdata->id);
    return NULL;
  }
  pthread_mutex_lock(&mutex1);		
  logFile << clientdata->id << ": Tunnel closed" << std::endl;		
  pthread_mutex_unlock(&mutex1);
  close(server_fd);
  close(client_fd);
  return NULL;
...
void proxy::run() {
  ...
  while (1) {
    ...
    clientdata->client_fd = client_fd;
